  ...
  while (table_dspic[n].name != NULL)
  {
    if ((opcode & table_dspic[n].mask) == table_dspic[n].opcode)
    {
      ...
      *cycles_max = table_dspic[n].cycles_max;
      ...
      {
        case OP_NONE:
          snprintf(instruction, length, "%s", table_dspic[n].name);
          return 4;
        case OP_F:
          f = opcode & 0xffff;
          snprintf(instruction, length, "%s 0x%04x", table_dspic[n].name, f);
          return 4;
        case OP_F_WREG:
          b = (opcode >> 14) & 1;
          d = (opcode >> 13) & 1;
          f = opcode & 0x1fff;
          snprintf(instruction, length, "%s%s 0x%04x%s", table_dspic[n].name, bflag[b], f, (d == 0 ? ", wreg" : ""));
          return 4;
        case OP_F_OR_WREG:
          b = (opcode >> 14) & 1;
          d = (opcode >> 13) & 1;
          ...
          if (d == 0)
          {
            snprintf(instruction, length, "%s%s wreg", table_dspic[n].name, bflag[b]);
          }
            else
          {
            snprintf(instruction, length, "%s%s 0x%04x", table_dspic[n].name, bflag[b], f);
          }
          return 4;
        case OP_WREG_F:
          b = (opcode >> 14) & 1;
          f = opcode & 0x1fff;
          snprintf(instruction, length, "%s%s wreg, 0x%04x", table_dspic[n].name, bflag[b], f);
          return 4;
        case OP_ACC:
          a = (opcode >> 15) & 1;
          snprintf(instruction, length, "%s %c", table_dspic[n].name, (a == 0) ? 'A' : 'B');
          return 4;
        case OP_ACC_LIT4_WD:
          b = (opcode >> 16) & 1;  // .R ?
          a = (opcode >> 15) & 1;
          ...
          if (lit == 0)
          {
            snprintf(instruction, length, "%s%s %c, %s", table_dspic[n].name, bflag[b], accum[a], temp);
          }
            else
          {
            snprintf(instruction, length, "%s%s %c, #%d, %s", table_dspic[n].name, bflag[b], accum[a], lit, temp);
          }
          return 4;
        case OP_ACC_LIT6:
          a = (opcode >> 15) & 1;
          ...
          snprintf(instruction, length, "%s %c, #%d", table_dspic[n].name, accum[a], lit);
          return 4;
        case OP_ACC_WB:
          a = (opcode >> 15) & 1;
          w = opcode & 0xf;
          snprintf(instruction, length, "%s %c, w%d", table_dspic[n].name, accum[a], w);
          return 4;
        case OP_BRA:
          offset = opcode & 0xffff;
          snprintf(instruction, length, "%s 0x%04x (%d)", table_dspic[n].name, (address / 2) + 2 + ((int32_t)(offset) * 2), ((int32_t)(offset) * 2));
          return 4;
        case OP_CP0_WS:
          b = (opcode >> 11) & 1;
        ...
        case OP_F_WND:
          ...
          w = opcode & 0xf;
          snprintf(instruction, length, "%s 0x%04x, w%d", table_dspic[n].name, f, w);
          return 4;
        case OP_GOTO:
          f = (opcode & 0xffff) | (memory->read32(address + 4) << 16);
          snprintf(instruction, length, "%s 0x%04x", table_dspic[n].name, f);
          return 8;
        case OP_LIT1:
          lit = opcode & 0x1;
          snprintf(instruction, length, "%s #%d", table_dspic[n].name, lit);
          return 4;
        case OP_LIT10_WN:
          lit = (opcode >> 4) & 0x3ff;
          b = (opcode >> 14) & 0x1;
          w = opcode & 0xf;
          snprintf(instruction, length, "%s%s #0x%x, w%d", table_dspic[n].name, bflag[b], lit, w);
          return 4;
        case OP_LIT14:
          lit = opcode & 0x3fff;
          snprintf(instruction, length, "%s #%d", table_dspic[n].name, lit);
        ...
        case OP_LIT14_EXPR:
          lit = opcode & 0x3fff;
          offset = memory->read32(address + 4);
          snprintf(instruction, length, "%s #%d, 0x%04x (%d)", table_dspic[n].name, lit, ((address / 2) + 2) + ((int32_t)(offset) * 2), ((int32_t)(offset) * 2));
          return 8;
        case OP_LIT16_WND:
          lit = (opcode >> 4) & 0xffff;
          w = opcode & 0xf;
          snprintf(instruction, length, "%s #0x%02x, w%d", table_dspic[n].name, lit, w);
          return 4;
        case OP_LIT8_WND:
          lit = (opcode >> 4) & 0xff;
          w = opcode & 0xf;
        ...
        case OP_LNK_LIT14:
          lit = opcode & 0x3fff;
          snprintf(instruction, length, "%s #0x%02x", table_dspic[n].name, lit);
          return 4;
        case OP_POP_D_WND:
          w = ((opcode >> 8) & 0x7) * 2;
          snprintf(instruction, length, "%s.d w%d", table_dspic[n].name, w);
          return 4;
        case OP_POP_S:
        ...
        case OP_UU_WB_LIT5_WND:
          ...
          snprintf(instruction, length, "%s%s w%d, #%d, w%d", table_dspic[n].name, temp2, w, lit, d);
          return 4;
        case OP_S_WM_WN:
        case OP_U_WM_WN:
          w = (opcode >> 7) & 0xf;
        ...
        case OP_WB_LIT4_WND:
          ...
          lit = opcode & 0xf;
          d = (opcode >> 7) & 0xf;
          snprintf(instruction, length, "%s w%d, #%d, w%d", table_dspic[n].name, w, lit, d);
          return 4;
        case OP_WB_LIT5:
          w = (opcode >> 11) & 0xf;
          lit = opcode & 0x1f;
          b = (opcode >> 10) & 0x1;
          snprintf(instruction, length, "%s%s w%d, #%d", table_dspic[n].name, bflag[b], w, lit);
          return 4;
        case OP_WB_LIT5_WD:
          w = (opcode >> 15) & 0xf;
          lit = opcode & 0x1f;
        ...
        case OP_WB_WN:
          ...
          d = opcode & 0xf;
          b = (opcode >> 10) & 0x1;
          snprintf(instruction, length, "%s%s w%d, w%d", table_dspic[n].name, bflag[b], w, d);
          return 4;
        case OP_WB_WNS_WND:
          w = (opcode >> 11) & 0xf;
          b = opcode & 0xf;
          d = (opcode >> 7) & 0xf;
          snprintf(instruction, length, "%s w%d, w%d, w%d", table_dspic[n].name, w, b, d);
          return 4;
        case OP_WB_WS:
          w = (opcode >> 11) & 0xf;
          get_wd(temp, sizeof(temp), opcode & 0xf, (opcode >> 4) & 0x7, 0);
        ...
        case OP_WN:
          w = opcode & 0xf;
          snprintf(instruction, length, "%s w%d", table_dspic[n].name, w);
          return 4;
        case OP_B_WN:
          w = opcode & 0xf;
          b = (opcode >> 14) & 1;
          snprintf(instruction, length, "%s%s w%d", table_dspic[n].name, bflag[b], w);
          return 4;
        case OP_DAW_B_WN:
          w = opcode & 0xf;
          snprintf(instruction, length, "%s.b w%d", table_dspic[n].name, w);
          return 4;
        case OP_WN_EXPR:
          offset = memory->read32(address + 4);
          w = opcode & 0xf;
          snprintf(instruction, length, "%s w%d, 0x%04x (%d)", table_dspic[n].name, w, ((address / 2) + 2) + ((int32_t)(offset) * 2), ((int32_t)(offset) * 2));
          return 8;
        case OP_WNS_F:
          w = opcode & 0xf;
          f = (((opcode >> 4) & 0x7fff) << 1);
          snprintf(instruction, length, "%s w%d, 0x%04x", table_dspic[n].name, w, f);
          return 4;
        case OP_WNS_WD_LIT10:
          ...
          snprintf(instruction, length, "%s%s w%d, [w%d%s%d]", table_dspic[n].name, bflag[b], w, d, (lit < 0) ? "" : "+", lit);
          return 4;
        case OP_WNS_WND:
          w = opcode & 0xf;
          d = (opcode >> 7) & 0xf;
        ...
        case OP_A_WX_WY_AWB:
          a = (opcode >> 15) & 0x1;
          snprintf(instruction, length, "%s %c", table_dspic[n].name, accum[a]);
          parse_dsp(instruction, opcode, 1);
          return 4;
        case OP_N_WM_WN_ACC_WX_WY:
          a = (opcode >> 15) & 0x1;
          d = (opcode >> 16) & 0x7;
          snprintf(instruction, length, "%s.n %s, %c", table_dspic[n].name, mmm_table[d], accum[a]);
        ...
        case OP_WM_WN_ACC_WX_WY:
          ...
          a = (opcode >> 15) & 0x1;
          d = (opcode >> 16) & 0x7;
          snprintf(instruction, length, "%s %s, %c", table_dspic[n].name, mmm_table[d], accum[a]);
          parse_dsp(instruction, opcode, 0);
          return 4;
  ...
  while (start < end)
  {
    ...
    if (count == 8)
    {
      fprintf(asm_context->list, "0x%04x: 0x%02x%02x%02x\n",
        (start / 2) + 2,
        asm_context->memory_read(start + 4),
        asm_context->memory_read(start + 5),
        asm_context->memory_read(start + 6));
    }
