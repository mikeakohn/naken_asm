...
{
  free_pools(defines_heap->memory_pool);
  defines_heap->memory_pool=NULL;
...
struct _memory_pool *memory_pool=defines_heap->memory_pool;
  ...
  if (memory_pool==NULL)
  {
    memory_pool=add_pool((struct _naken_heap *)defines_heap, DEFINES_HEAP_SIZE);
  }
  ...
  while(1)
  {
     ...
     {
       memory_pool->next=add_pool((struct _naken_heap *)defines_heap, DEFINES_HEAP_SIZE);
     }
  ...
  memory_pool->buffer[memory_pool->ptr-1]=(unsigned char)param_count;
